...
for(i in 1:n){
  ...
  Ymean1[,i] <- ifelse( zi==2, 2, -1 )*2*te + (tempg11/10)*te + (tempu1+tempg22/10) * fval1 + (tempu2+tempg21/10) * fval2 + tempu3 * fval3
  Ymean2[,i] <- ifelse( zi==1, 1, -1 ) * 2 + ifelse( zi==3, 1, 0 ) * 1*te + tempg11/10 + (tempu6+tempg32/10) * fval6 + (tempu5+tempg31/10) * fval5 
  Ymean3[,i] <- zi * (1*te+1/2) +tempu8 * fval8 + tempu9 * fval9
}
