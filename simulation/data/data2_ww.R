...
for(i in 1:sum(nts)){ 
  ...
  tempu2 <- rnorm(1,    ifelse( (zi==2&&zi==3), -zi+5+1, 0 ) + ifelse( (zi!=2&&zi!=3), zi+1, 0 ),  1.5)
  tempu3 <- rnorm(1,    (6-zi),  1)
  tempu4 <- rnorm(1,    (6-zi),  1)
  ...
  Ymean1[,i] <- (3*te+3)*ifelse( (zi!=2&&zi!=4), zi, -zi ) + tempu1 * fval1 + tempu2 * fval2 + tempu3 * fval3
  Ymean2[,i] <- ((10)*te+3)*zi + tempu4 * fval4 + tempu5 * fval5 + tempu6 * fval6
}
