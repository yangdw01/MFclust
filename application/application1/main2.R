...
# langitude & longitude & socio-economic
containCOV <- 1
containCAT <- ifelse( length(temp_list2)>0,1,0 )
containMULT <- ifelse( length(funname)==1, 0, 1 )

cov_list <- c(geo_list, temp_list1)
Xdat <- t( mcc.indicators[citynum_mcc,cov_list] )
Xdat <- t(sapply(1:nrow(Xdat), function(x){ (Xdat[x,]-mean(Xdat[x,]))/sd(Xdat[x,]) }))
Xdat <- Xdat[1:4,]


if( containCAT == 1 ){
  
  Zdat <- tempZdat <- mcc.indicators[citynum_mcc,temp_list2]
  for(iwe in unique(tempZdat)){ Zdat[tempZdat==iwe] <- which( unique(tempZdat) %in% iwe ) }
  Zdat <- t( Zdat )
}


set.seed(1800) #  1700 1800 2200 2300 3500 4000
source("preprocess.R")
...
plot(exp(msc3[,2])-1, type='l', ylim=c(5,18), main=expression("(c) MFclust0 - PM2.5"), ylab=expression(paste("PM2.5 (", mu, "g/", m^3, ")")), xlab='Month', xaxt='n')
axis(1, monthnum, monthnames, labels = F)
text(monthnum, par("usr")[3] - 0.4, labels = monthnames, srt = 45, pos = 1, xpd = TRUE, cex=0.85)
for(i in sort(unique(clusters2))){ lines( exp(msc3[,clusters[which(clusters2 == i)[1]]])-1, col=i, lwd=3) }
...
# 
# 
# plot(exp(msc3[,2])-1, type='l', ylim=c(0,20), main=expression("(c) MFclust0 - PM2.5"), ylab=expression(paste("PM2.5 (", mu, "g/", m^3, ")")), xlab='Month', xaxt='n')
# axis(1, monthnum, monthnames, labels = F)
# text(monthnum, par("usr")[3] - 0.4, labels = monthnames, srt = 45, pos = 1, xpd = TRUE, cex=0.85)
# for(i in sort(unique(clusters2))){
# 
...
# 
# 
# plot(msc3[,2], type='l', ylim=quantile(Ydat[,3], c(0,1)), main=expression("(c) MFclust0 - PM2.5"), ylab=expression(paste("PM2.5 (", mu, "g/", m^3, ")")), xlab='Month', xaxt='n')
# axis(1, monthnum, monthnames, labels = F)
# text(monthnum, par("usr")[3] - 0.4, labels = monthnames, srt = 45, pos = 1, xpd = TRUE, cex=0.85)
# for(i in sort(unique(clusters2))){
# 
...
# original_Xdat <- mcc.indicators[citynum_mcc,cov_list]
# plot( jitter(original_Xdat[,3], factor=10), jitter(original_Xdat[,4], factor=10), cex=2, pch=3, lwd=2, col=tempresult2, xlab="GDP ($)", ylab="Unemp (%)", main="(i) Socio-economic variables", cex.main=1.5, cex.lab=1.2 )
# legend(58700,13.6, legend = c("cluster 1", "cluster 2", "cluster 3", "cluster 4", "cluster 5"), col=c(1,2,4,3,5), pch = c(3), cex=1.5)
# 
# text( original_Xdat[2,3]-1200, original_Xdat[2,4]-0.5, mcc_cityname[2], cex=1, col=clusters2[2]  )
# text( original_Xdat[3,3]-1200, original_Xdat[3,4]+0.5, mcc_cityname[3], cex=1, col=clusters2[3]  )
# text( original_Xdat[16,3], original_Xdat[16,4]-0.5, mcc_cityname[16], cex=1, col=clusters2[16]  )
# text( original_Xdat[20,3], original_Xdat[20,4]+0.5, mcc_cityname[20], cex=1, col=clusters2[20]  )
# 
# 
# matrix.heatmap(temp_ppm[order(clusters),order(clusters)])
# 
...
tempclustrs <- clusters2

clusters2[tempclustrs == 1] <- 4
clusters2[tempclustrs == 3] <- 1




plot(msc1[,1], type='l', ylim=c(8,35), main=expression("(e) MFclust0 - O"[3]), ylab=expression("O"[3] * " (ppb)"), xlab='Month', xaxt='n')
axis(1, monthnum, monthnames, labels = F)
text(monthnum, par("usr")[3] - 1, labels = monthnames, srt = 45, pos = 1, xpd = TRUE, cex=0.85)
for(i in sort(unique(clusters2))){
...
plot(msc2[,1], type='l', ylim=c(5,27), main=expression("(f) MFclust0 - NO"[2]), ylab=expression("NO"[2] * " (ppb)"), xlab='Month', xaxt='n')
axis(1, monthnum, monthnames, labels = F)
text(monthnum, par("usr")[3] - 1, labels = monthnames, srt = 45, pos = 1, xpd = TRUE, cex=0.85)
for(i in sort(unique(clusters2))){
...
plot(exp(msc3[,1])-1, type='l', ylim=c(3,18), main=expression("(g) MFclust0 - PM2.5"), ylab=expression(paste("PM2.5 (", mu, "g/", m^3, ")")), xlab='Month', xaxt='n')
axis(1, monthnum, monthnames, labels = F)
text(monthnum, par("usr")[3] - 0.4, labels = monthnames, srt = 45, pos = 1, xpd = TRUE, cex=0.85)
for(i in sort(unique(clusters2))){
...
points( mcc.indicators[citynum_mcc,geo_list[2]], mcc.indicators[citynum_mcc,geo_list[1]], pch=3, col=clusters2, lwd=2,cex=1.7 )
# text( mcc.indicators[citynum_mcc,geo_list[2]], mcc.indicators[citynum_mcc,geo_list[1]], 1:n, cex=0.8, col=tempresult )
title(main="(h)", cex.main=1.5)
# legend(-63, 80, legend = c("cluster 1", "cluster 2", "cluster 3"), col=c(1,2,4), pch = c(3,3,3), cex=1.5)
text( ((mcc.indicators[citynum_mcc,geo_list[2]]))[2], ((mcc.indicators[citynum_mcc,geo_list[1]]))[2]-1.2, mcc_cityname[2], cex=1, col=clusters2[2]  )
text( ((mcc.indicators[citynum_mcc,geo_list[2]]))[3], ((mcc.indicators[citynum_mcc,geo_list[1]]))[3]+2, mcc_cityname[3], cex=1, col=clusters2[3] )
text( ((mcc.indicators[citynum_mcc,geo_list[2]]))[16]-6, ((mcc.indicators[citynum_mcc,geo_list[1]]))[16]+2, mcc_cityname[16], cex=1, col=clusters2[16]  )
segments( ((mcc.indicators[citynum_mcc,geo_list[2]]))[20], ((mcc.indicators[citynum_mcc,geo_list[1]]))[20],
...
par(mar=c(5.1, 4.1, 4.1, 9.1), xpd=TRUE)
original_Xdat <- mcc.indicators[citynum_mcc,cov_list]
plot( jitter(original_Xdat[,3], factor=10), jitter(original_Xdat[,4], factor=10), cex=2, pch=3, lwd=2, col=clusters2, xlab="GDP ($)", ylab="Unemp (%)", main="(i) Socio-economic variables", cex.main=1.5, cex.lab=1.2 )
legend(58700,13.6, legend = c("cluster 1", "cluster 2", "cluster 3"), col=c(1,2,4), pch = c(3), cex=1.5)
...
matrix.heatmap(temp_ppm[order(clusters),order(clusters)], main = "(b) MFclust1 - Heatmap")
