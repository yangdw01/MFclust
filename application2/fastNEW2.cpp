...
// [[Rcpp::export]]
mat getPSIis( int n, vec Sigmas, int tempK, int H, mat LFLM, mat Eta, mat hatPSIis, mat Svaris ){   //////
  
  mat PSIis = zeros<mat>(H,n);
  mat LFLMk = LFLM.rows(tempK,tempK+H-1);
  vec Sigmask = Sigmas.subvec(tempK,tempK+H-1);
  mat inv_Sigma = diagmat(1.0/Sigmask);
  
  for(int i=0; i < n; ++i){
    
    mat Si = Svaris.cols( i*H, (i+1)*H-1 );
    mat invSi = armaInv(Si);
    
    vec hatPSIi = hatPSIis.col(i);
    vec Etai = Eta.col(i);
    
    mat tempsig = inv_Sigma + invSi;
    tempsig = armaInv(tempsig);
    
    vec tempmean = invSi * hatPSIi + inv_Sigma * LFLMk * Etai;
    tempmean = tempsig * tempmean;
    
    PSIis.col(i) = conv_to<vec>::from( rmvnorm(1, tempmean, tempsig) );
  }
...
// [[Rcpp::export]]
mat getTHETA_2( int cc, vec kkind, int n, mat nobs, mat B, vec Sigmas, vec KKs, mat A, mat LFLM, mat Eta, mat Ydat ){  //////
  ...
  vec ind = cumsum(nobs.col(0));
  vec ind0 = {0};
  ind = join_vert(ind0, ind);
  
  mat LFLM1 = LFLM.rows(0,Ksum-1);
  vec Sig1 = Sigmas.subvec(0, Ksum-1);
  mat invSig1 = diagmat(1.0/Sig1);
  mat invA = armaInv(A);
  
  for(int i=0; i<n; ++i){
    
    mat Ydati = Ydat.rows(ind(i), ind(i+1)-1);
    vec ydati = matTOvec(Ydati);
    vec Etai = Eta.col(i);
    
    mat Bi=B.rows(ind(i), ind(i+1)-1);
    mat BBi = zeros<mat>(TTs(i)*cc, sum(KKs));
    for(int h=0; h < cc; ++h){
      
      mat BBih = Bi.cols(KKsind(h), KKsind(h+1)-1);
      BBi.submat( h*TTs(i), KKsind(h) ,(h+1)*TTs(i)-1, KKsind(h+1)-1 ) = BBih;
    }
    
    mat tempvar = armaInv( invSig1 + BBi.t() * kron( invA, diagmat( ones<vec>(TTs(i)) ) ) * BBi );
    vec tempmean = conv_to<vec>::from( tempvar * ( invSig1*LFLM1*Etai + BBi.t()*kron( invA, diagmat( ones<vec>(TTs(i)) ) )*ydati ) );
  
    THETA.col(i) = conv_to<vec>::from( rmvnorm(1, tempmean, tempvar) );
  }
...
// [[Rcpp::export]]
mat getA_2( int n, double rhoA, mat WA, int cc, mat nobs, vec KKs, mat THETAcs, mat Ydat, mat B ){
  ...
  double temprho = rhoA + sum(TTs);
  vec TTsind = cumsum(TTs);
  vec TTsind0 = {0};
  TTsind = join_vert(TTsind0, TTsind);
  ...
  for( int i=0; i<n; ++i){
    ...
    vec thetai = THETAcs.col(i);
    vec ei = ydati - BBi * thetai;
    mat Ei = vecTOmat(ei, TTs(i), cc);
    
    tempW += Ei.t() * Ei;
  }
  
  mat result = armaInv( rwish(temprho, armaInv(tempW)) );
...
// [[Rcpp::export]]
vec getCLUST_2( vec burnthin, int numiter, double eps,
              int n, int M, int tempKpq, int tempKp, int tempK, double ppi, double llamb, int cc, int containBETA, int containCOV, int containCAT,
              double Hp, vec kkind, mat nobs, mat B, vec KKs, mat Ydat, mat Xdat, mat Zdat, int qq, vec qdims, vec qind, vec qind1,
              double alpha, double beta, mat ZETA, vec LL, mat ZZ, mat VV, mat Eta, vec ums, vec Sigmas, vec mm, vec wms, mat Zmu, vec PP, double nu, 
              mat A, double psi0, mat tempTHETA, mat gis,
              double a_u, double b_u, double a_w, double b_w, double a_alpha, double b_alpha, 
              double a_beta, double b_beta, double a_nu, double b_nu, double a_sigma, double b_sigma, double a_psi, double b_psi, double rhoA, mat WA ){
  ...
  for(int i=0; i<numiter; ++i){
    ...
    if(cc>1){
      ...
      tempTHETA1 = getTHETA_2( cc, kkind, n, nobs, B, Sigmas1, KKs, A1, LFLM1, Eta1, Ydat );
      A1 = getA_2( n, rhoA, WA, cc, nobs, KKs, tempTHETA1, Ydat, B );
