...
if( CASE == 1 ){
  ...
  if(containCAT == 1){
    ...
    ZZ0 <- matrix( rbinom( (tempKH+(qstar-qq))*r, 1, 1/2 ), (tempKH+(qstar-qq)), r )
    VV0 <- matrix( rnorm( (tempKH+(qstar-qq))*r ), (tempKH+(qstar-qq)), r )
    Eta0 <- matrix( rnorm( n*r ), r, n )
    
    THETA0 <- (ZZ0 * VV0) %*% Eta0
    THETAcs0 <- list( THETA0[1:K1,], THETA0[(1:K2)+K1,] )
    Sigmas0 <- 1/rgamma(tempKH+(qstar-qq), a_sigma, b_sigma)
    
  }else{
    
    ZZ0 <- matrix( rbinom( tempKH*r, 1, 1/2 ), tempKH, r )
    VV0 <- matrix( rnorm( tempKH*r ), tempKH, r )
    Eta0 <- matrix( rnorm( n*r ), r, n )
    
    THETA0 <- (ZZ0 * VV0) %*% Eta0
    THETAcs0 <- list( THETA0[1:K1,], THETA0[(1:K2)+K1,] )
    Sigmas0 <- 1/rgamma(tempKH, a_sigma, b_sigma)
  }
  ...
  # hyperparameter
  a_alpha <- b_alpha <- a_beta <- b_beta <- a_u <- b_u <- a_w <- b_w <- a_sigma <- b_sigma <- a_psi <- b_psi <- a_nu <- b_nu <-  1
  ...
  if(containCAT == 1){
    ...
    ZZ0 <- matrix( rbinom( (tempKH+p+(qstar-qq))*r, 1, 1/2 ), (tempKH+p+(qstar-qq)), r )
    VV0 <- matrix( rnorm( (tempKH+p+(qstar-qq))*r ), (tempKH+p+(qstar-qq)), r )
    Eta0 <- matrix( rnorm( n*r ), r, n )
    
    THETA0 <- (ZZ0 * VV0) %*% Eta0
    THETAcs0 <- list( THETA0[1:K1,], THETA0[(1:K2)+K1,] )
    Sigmas0 <- 1/rgamma((tempKH+p+(qstar-qq)), a_sigma, b_sigma)
    
  }else{
    
    ZZ0 <- matrix( rbinom( (tempKH+p)*r, 1, 1/2 ), (tempKH+p), r )
    VV0 <- matrix( rnorm( (tempKH+p)*r ), (tempKH+p), r )
    Eta0 <- matrix( rnorm( n*r ), r, n )
    
    THETA0 <- (ZZ0 * VV0) %*% Eta0
    THETAcs0 <- list( THETA0[1:K1,], THETA0[(1:K2)+K1,] )
    Sigmas0 <- 1/rgamma((tempKH+p), a_sigma, b_sigma)
  }
